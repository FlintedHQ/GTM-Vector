  ...
  <nav className="fixed top-0 left-0 right-0 z-50 py-6 px-6 md:px-12 bg-black/40 backdrop-blur-xl border-b border-white/5">
    <div className="max-w-7xl mx-auto flex items-center justify-between">
      ...
      <div className="hidden md:flex items-center gap-10 text-xs font-bold uppercase tracking-widest text-gray-500">
        ...
        <a href="#stack" className="hover:text-lime-500 transition-colors">Stack</a>
...
const ServiceCard = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <div className="glass-card p-10 rounded-[32px] group relative overflow-hidden flex flex-col items-start gap-5">
    ...
    <h3 className="text-2xl font-bold group-hover:text-lime-500 transition-colors tracking-tight">{title}</h3>
...
const FAQItem = ({ question, answer }: { question: string, answer: string }) => {
  ...
  return (
    <div className="border-b border-white/5 last:border-0 overflow-hidden">
      ...
      >
        <span className={`text-lg font-bold transition-all ${isOpen ? 'text-lime-500 tracking-wide' : 'text-gray-300'}`}>{question}</span>
        <div className={`p-2 rounded-full border transition-all duration-500 ${isOpen ? 'border-lime-500 rotate-180 bg-lime-500 text-black' : 'border-white/10 text-gray-500'}`}>
           <ChevronDown size={18} />
        </div>
      </button>
      <div className={`transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${isOpen ? 'max-h-60 pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
        <p className="text-gray-400 leading-relaxed font-medium pl-2 border-l-2 border-lime-500/20">{answer}</p>
      </div>
    </div>
  );
};
    ...
    <div className="min-h-screen relative animated-bg selection:bg-lime-500 selection:text-black">
      ...
      <main className="relative z-10 pt-32 px-6">
        ...
        <section className="max-w-7xl mx-auto py-24 md:py-40 text-center">
          ...
          <h1 className="text-6xl md:text-[100px] font-jakarta font-extrabold tracking-tighter mb-10 leading-[0.9] text-glow">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-lime-400 via-emerald-400 to-lime-500">Built to Scale</span>
          </h1>
        ...
        <section className="max-w-7xl mx-auto mb-40">
          <div className="relative group overflow-hidden rounded-[40px] p-[2px] bg-gradient-to-b from-white/10 to-transparent">
            <div className="bg-black/90 backdrop-blur-3xl rounded-[38px] py-16 px-8 flex flex-col items-center text-center">
              <span className="text-7xl md:text-[120px] font-jakarta font-black text-white mb-2 tracking-tighter text-glow">+73%</span>
              <p className="text-2xl md:text-4xl font-jakarta font-bold text-gray-200 tracking-tight">booked meetings on average.</p>
              <div className="h-1 w-24 bg-lime-500 mt-8 mb-4 rounded-full opacity-50" />
              <p className="text-[10px] text-gray-600 uppercase tracking-[0.2em] font-black italic">Varies by ICP, offer, and volume.</p>
            </div>
          </div>
        </section>
        ...
        <section id="services" className="max-w-7xl mx-auto py-24 mb-24">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
            <div className="max-w-xl">
              ...
              <h2 className="text-4xl md:text-6xl font-jakarta font-bold tracking-tight mb-8">Architecting Growth</h2>
              <p className="text-gray-400 text-lg leading-relaxed">We don't just "set up tools." We engineer high-velocity revenue engines focused on precision and deliverability.</p>
            </div>
        ...
        <section id="process" className="max-w-5xl mx-auto py-24 mb-24 relative">
          <div className="text-center mb-24">
            ...
            <h2 className="text-4xl md:text-6xl font-jakarta font-bold tracking-tight mb-8">The Vector Blueprint</h2>
              ...
              <div key={i} className="flex flex-col md:flex-row gap-10 group relative">
                ...
                <div className="glass-card flex-1 p-8 rounded-3xl border-l-4 border-l-lime-500/30 group-hover:border-l-lime-500">
                  <div className="flex flex-wrap items-center gap-4 mb-4">
                    ...
                    <span className="px-4 py-1.5 rounded-lg bg-white/5 border border-white/10 text-lime-400 text-[10px] font-black uppercase tracking-widest">{item.duration}</span>
                  </div>
        ...
        <section id="faq" className="max-w-4xl mx-auto py-24 mb-40">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-jakarta font-bold tracking-tight">Intelligence Briefing</h2>
          </div>
        ...
        <section className="max-w-7xl mx-auto py-24 mb-48">
          <div className="relative group p-[2px] rounded-[48px] overflow-hidden">
            ...
            <div className="relative bg-[#02040a] rounded-[46px] py-32 px-10 text-center flex flex-col items-center">
              <h2 className="text-5xl md:text-8xl font-jakarta font-black mb-10 tracking-tighter leading-[0.9]">Ready to Upgrade?</h2>
              <p className="text-2xl text-gray-400 mb-16 max-w-2xl font-medium">Stop battling friction. Start deploying architecture. Secure your technical audit today.</p>
              <div className="flex flex-col md:flex-row items-center gap-8">
                 ...
                 >
                   <Mail size={22} className="group-hover:text-lime-500 group-hover:scale-110 transition-all" /> info@gtmvector.com
                 </a>
              </div>
            </div>
          </div>
      ...
      <footer className="relative z-10 py-16 px-6 border-t border-white/5 bg-black">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          ...
          <p className="text-xs font-bold text-gray-600 uppercase tracking-[0.3em]">© 2024 GTM Vector Architecture. Engineering for revenue.</p>
          <div className="flex items-center gap-10 text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
             ...
             <a href="#" className="hover:text-lime-500 transition-colors">Terms</a>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative w-full max-w-xl bg-[#05070a] border border-white/10 rounded-[40px] p-10 md:p-14 shadow-2xl animate-in zoom-in duration-500">
             ...
             <p className="text-gray-400 mb-10 text-lg font-medium leading-relaxed">Secure your slot. We review your architecture within a 48-hour window.</p>
             <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); alert('System update: Audit request dispatched.'); }}>
                <div>
                  <label className="block text-[10px] font-black text-gray-500 mb-3 uppercase tracking-[0.2em]">Deployment Email</label>
                  <input type="email" required placeholder="name@company.com" className="w-full px-5 py-5 bg-white/5 border border-white/10 rounded-2xl focus:outline-none focus:border-lime-500 transition-all text-white font-bold" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-500 mb-3 uppercase tracking-[0.2em]">Intelligence Link (LinkedIn)</label>
                  <input type="text" placeholder="linkedin.com/in/..." className="w-full px-5 py-5 bg-white/5 border border-white/10 rounded-2xl focus:outline-none focus:border-lime-500 transition-all text-white font-bold" />
                </div>
                <div>
                   <label className="block text-[10px] font-black text-gray-500 mb-3 uppercase tracking-[0.2em]">Personnel Count</label>
                ...
                <button type="submit" className="w-full py-6 bg-lime-500 text-black font-black rounded-2xl hover:bg-white transition-all text-xl shadow-2xl">Confirm Deployment</button>
             </form>
             <button 
              onClick={() => setIsModalOpen(false)}
      ...
      {/* Cursor proximity effect (Badass Pointer) */}
      <div 
